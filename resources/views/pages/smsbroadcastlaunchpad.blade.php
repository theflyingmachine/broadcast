...
<button type="button" id="button1" class="btn btn-success btn-lg btn-block"> Broadcast </button>
  ...
  <script >
    ...
    error: function (data, textStatus, errorThrown) {
		...
		document.getElementById('loadarea').src = 'smsbroadcast';
...
@foreach($allbroadcasts as $broadcast)
    ...
    <div class="card">
      <img class="card-img-top" src="https://wallpapercave.com/wp/ZtpS25u.jpg" alt="Card image cap">
      <div class="card-body">
        <h4 class="card-title">{{$broadcast->name}}</h4>
        <p class="card-text">{{$broadcast->mobile}}</p>
        <p class="card-text">{{$broadcast->template}}</p>
      </div>
